...
/**
 * MyBatis를 통해 데이터를 가져올 때, CLOB, BLOB 등 일부 컬럼의 데이터가 NULL일 경우 오류가 발생하여 log4jdbc-log4j2 일부 클래스 수정
 * 
 * @author jhkang
 * @since 2015.08.24
 */
public class DefaultResultSetCollector implements ResultSetCollector {
	...
	/**
	 * A {@code boolean} defining whether parameters of this
	 * {@code DefaultResultSetColector} has already been obtained from a
	 * {@code ResultSetMetaData}.
	 */
	private boolean loaded;
	
	/**
	 * An <code>int</code> representing the number of columns in the real
	 * <code>ResultSet</code> object, obtained by calling
	 * <code>getColumnCount</code> on the related <code>ResultSetMetaData</code>
	 * object. This attribute is an <code>Integer</code> rather than an
	 * <code>int</code>, so that we can distinguish the case where the column
	 * count has not yet been obtained, from the case when the column count is 0
	 ...
	 * the index of a column, and the corresponding value is a
	 * <code>String</code> being the label of the column. This <code>Map</code>
	 * is populated by calling <code>getColumnLabel</code> on the
	 * <code>ResultSetMetaData</code> object of the real <code>ResultSet</code>
	 * object, for each column (see {@link #columnCount}).
	 */
	 ...
	 * A <code>Map</code> where the key is an <code>Integer</code> representing
	 * the index of a column, and the corresponding value is a
	 * <code>String</code> being the name of the column. This <code>Map</code>
	 * is populated by calling <code>getColumnLabel</code> on the
	 * <code>ResultSetMetaData</code> object of the real <code>ResultSet</code>
	 * object, for each column (see {@link #columnCount}).
	 */
	...
	/**
	 * A <code>boolean</code> that is the last value returned by a call to
	 * <code>next</code> (or <code>first</code>) on the related
	 * <code>ResultSet</code>. This allows to know if this
	 * <code>ResultSetCollector</code> must be printed when <code>close</code>
	 * is called on the related <code>ResultSet</code>: if the previous call to
	 * <code>next</code> returned <code>true</code>, then this
	 * <code>ResultSetCollector</code> must be printed on the call to
	 * <code>close</code>; otherwise, it was already printed following the
	 * previous call to <code>next</code>, and should not be printed on the call
	 * to <code>close</code>.
	 * <p>
	 * This attribute must not be reset after a printing (the other attributes
	 * are, through a call to <code>reset</code>), in case the related
	 * <code>ResultSet</code> is reused.
	 * <p>
	 * Default value at initialization is <code>true</code> (
	 * <code>ResultSetCollector</code> not printed).
	 */
	...
	private static final List<String> GETTERS = Arrays.asList(new String[] { "getArray", "getAsciiStream",
			"getBigDecimal", "getBinaryStream", "getBlob", "getBoolean", "getByte", "getBytes", "getCharacterStream",
			"getClob", "getDate", "getDouble", "getFloat", "getInt", "getLong", "getNCharacterStream", "getNClob",
			"getNString", "getObject", "getRef", "getRowId", "getShort", "getSQLXML", "getString", "getTime",
			"getTimestamp", "getUnicodeStream", "getURL" });
	...
	public boolean methodReturned(ResultSetSpy resultSetSpy, String methodCall, Object returnValue, Object targetObject, Object... methodParams) {
		...
		// TODO: Tim: if prev() called, warn about no support for reverse
		// cursors
	...
	public void preMethod(ResultSetSpy resultSetSpy, String methodCall, Object... methodParams) {
		if ((methodCall.equals("next()") || methodCall.equals("close()")) && fillInUnreadValues) {
			if (row != null) {
				int colIndex = 0;
				for (Object v : row) {
